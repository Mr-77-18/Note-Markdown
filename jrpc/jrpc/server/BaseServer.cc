        ...
        :server_(loop, listen)
{
    ...
    server_.setMessageCallback(std::bind(//关注一下这个函数,这个是真实数据来往处理的函数
            &BaseServer::onMessage, this, _1, _2));
}

//设置成为tcpserver的connetioncallback
template <typename ProtocolServer>
void BaseServer<ProtocolServer>::onConnection(const TcpConnectionPtr& conn)
{
    if (conn->connected()) {//连接成功
        DEBUG("connection %s is [up]",conn->peer().toIpPort().c_str());
...
//Channel::handleEvent()->TcpConnection::handleRead()->BaseServer::onMessage();
template <typename ProtocolServer>
void BaseServer<ProtocolServer>::onMessage(const TcpConnectionPtr& conn, Buffer& buffer)
{
    try {
        handleMessage(conn, buffer);//处理都过来的数据
    }
...
//TcpConnection::handleRead()->BaseServer::onMessage()->BaseServer::handleMessage();
template <typename ProtocolServer>
void BaseServer<ProtocolServer>::handleMessage(const TcpConnectionPtr& conn, Buffer& buffer)
{
    while (true) {
        ...
        //将这些数据传给RpcServer,convert将当前对象转换成rpcserver对象
